...
libm. `regex.data` is the same shape from `tools/mkregexdata.py`, which asks the
host's own `<regex.h>` through ctypes: 2,211 patterns and subjects, three eflags
each. `zlib.data` is the same again, from `tools/mkzlibdata.py`. It holds the
length and CRC-32 of each stream the host's own zlib made: eight inputs across
the levels, strategies, windows and memory levels, 300 cases or so.
...
every entry through `web/fs.js`'s `DecompressionStream` and compares digests.
It also inflates every deflated entry with `zlib_uncompress` and compares the
result against that answer. The data is Python's deflate, inflated two ways,
across the whole archive.

**What `regex.data` cannot hold is written out in the case.** A generated table
can only carry what every host agrees about, and three things it does not: the
BSDs have no back-references in an ERE where GNU does, `\|` is a literal bar
here and alternation in glibc, and `.` takes a whole UTF-8 sequence here where
...
direction unremarked. There are none now: the subexpression-assignment family
is gone, the matcher having been given POSIX's rule rather than a
backtracker's, and the two collating-element absences beside it are
implemented. The table stays, empty, because an entry is how the next one would
be recorded. A re-sync is a copy plus the two wrapper lines; it will shift line
