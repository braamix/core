...
Reasoning, alternatives and trade-offs behind the code released as 0.10.
...
## 0.10 — The libraries a port links, and the escapes a guest speaks

`BRAAM_VERSION_BASE` moves to 0.10; the commit count and the hash behind it
carry on unedited. 0.8 was written from the point of view of the *page* and 0.9
from that of the *command line*. 0.10 is written from the point of view of a
program that was **not written here**: what it can link, what it can print,
what it can boot into, and what it can wait on. Five libraries arrive in the
SDK, the screen learned to read the escape sequences such a program emits and
the process runtime to send the ones it reads back, a site can boot into a
program instead of a shell, and one syscall answers the question a program with
two pipes cannot answer for itself.

**`PROC_ABI` moves to 21, and `Sys::Poll` is the whole of it.** Every stamped
binary must be rebuilt and every repository re-signed; a package built against
the 0.9 SDK is refused at exec. An added operation alone has been free since
`Truncate` — the number refuses a *stale* binary, and one that never issues an
operation cannot tell it is missing. What is not free is `Error::Busy`, a
sixteenth error value an older process runtime has no name for, and the shape
of the failure without a bump: a program built against this SDK that polls on a
0.9 kernel would be told `Unsupported` in the middle of a wait rather than
refused at exec, where a version mismatch belongs.

**Five libraries, and the rule that decided each one.** `braam::regex` is
POSIX regular expressions, lifted out of the `eh` editor in the applications
tree and finished — a BRE arm, `regerror`, `REG_ICASE`, `REG_NOSUB` and
`REG_STARTEND`, and leftmost-longest subexpressions — with AT&T's `regex(3)`
conformance corpus as its oracle, which is the one oracle in the tree that is
POSIX's rather than some host's. `braam::zlib` and `braam::bzip2` are
**rewritten** in C++, because their C is small enough to translate and the
translation is checked against the host's own library byte for byte.
`braam::lzma` and `braam::zstd` are **vendored** verbatim, because 35,000 and
50,000 lines of C are not a rewrite anybody should review; six renamed
allocator and string functions are the whole of what they stand on, and the
re-sync is a copy with no edit at all. The dividing line is size, and the test
is the same for all four: what this tree compresses is what the host's library
compresses, to the byte.

**The screen parses escapes, and the grid is still the model.** §2's invariant
said no parser paints the grid; it now says ANSI is **one encoding into** the
grid rather than the model of it. `screen_write` understands the sequences in
[ANSI_Escape_Codes.md](../ANSI_Escape_Codes.md) §4 and swallows the rest, so a
guest written for a terminal draws something rather than nothing, while
everything native still writes cells. The other half is a program's:
`key_encode` lives in the *process* runtime, because what a program sends for an
arrow key is the program's business and no part of the kernel's. Nothing in this
tree emits either — the two tables exist for guests.

**A site can boot into a program.** `/etc/init` is one line in a boot archive
naming what terminal 0 runs; absent, empty or unreadable all mean the shell, and
a named program also keeps boot's banner off the grid. A file rather than a
mount option, so a page cannot claim a program the archive does not carry.

**And `Sys::Poll`, which is where the release started.** A program could park on
exactly one thing, and `Read` is the only way to learn that a pipe has bytes —
so a program holding a child's stdout and its stderr deadlocked when the child
filled the one it was not draining. Poll is one waiter armed on every channel
named, and writing it turned out a scheduler bug that nothing had been able to
reach and a missing guard on descriptors 0, 1 and 2 that would have let a user
program panic the kernel.

**What else moved.** The allocator's size classes are powers of two up to half a
span, `PACKAGE_MAX` is 50 MiB, the compile cache keys on the image rather than
the path it came from — so an upgraded package runs the bytes it installed — the
SDK ships the test harness, the kernel and the boot archive, and the build
refuses a Node older than 22.12 at configure time rather than failing oddly
later.
...
the system case drives all four. `polltest two` is the shape a real program
would write: two children, two pipes, and a line printed as it arrives rather
than one pipe drained before the other is looked at.
