// wcwidth. The intervals and the rules are Markus Kuhn's, Unicode 5.0:
//
...
//
// The combining set is `uniset +cat=Me +cat=Mn +cat=Cf -00AD +1160-11FF
// +200B c`. The East Asian Ambiguous variant has no caller and is not here.
//
// This is the one place the kit contradicts the system: the grid is one Cell
// per rune (kernel/screen.h), so a caller counting columns with this will
// disagree with the screen about a wide one. doc/Compat.md says so.
#include <wchar.h>
...
{
    constexpr unsigned N = sizeof(COMBINING) / sizeof(COMBINING[0]);
    if (c < COMBINING[0].first || c > COMBINING[N - 1].last)
        return false;
...
// East Asian Wide and Fullwidth. Upstream's disjunction as a table; its
// "0x2e80..0xa4cf except 0x303f" is the two rows that skip that point.
const Interval WIDE[] = {
...
bool wide(unsigned c)
{
    for (const Interval &v : WIDE)
        if (c >= v.first && c <= v.last)
            return true;
    return false;
}
