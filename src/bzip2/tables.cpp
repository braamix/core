// bzip2's two tables, after crctable.c and randtable.c, and its CRC.
#include "bzip2/bzlib_private.h"

namespace {

constexpr BzCrcTable make_crc_table()
{
    BzCrcTable t{};
    for (u32 i = 0; i < 256; i++) {
        u32 c = i << 24;
...
static_assert(make_crc_table().v[1] == 0x04c11db7U && make_crc_table().v[255] == 0xb1f740b4U,
              "crctable.c's table");

} // namespace

constexpr BzCrcTable BZ_CRC_TABLE = make_crc_table();

// The pseudo-random run lengths of a randomised block: bzip2 0.9.0 wrote
// these, and a decoder reads them still.
const i32 BZ_RAND_NUMS[512] = {
