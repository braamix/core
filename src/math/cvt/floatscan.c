// Derived from musl's src/internal/floatscan.c (MIT), what answers strtod
// there. Long double becomes double, the one L-suffixed literal and the four
// l-suffixed calls go with it, and the FILE/shgetc scanner becomes cvt.h's Cur.
#include "cvt.h"
...
static long long scanexp(Cur *f, int pok)
{
	int c;
	int x;
	long long y;
...
static double decfloat(Cur *f, int c, int bits, int emin, int sign, int pok)
{
	uint32_t x[KMAX];
...
static double hexfloat(Cur *f, int bits, int emin, int sign, int pok)
{
	uint32_t x = 0;
	double y = 0;
	double scale = 1;
...
double __floatscan(Cur *f, int prec, int pok)
{
	int sign = 1;
	size_t i;
	int bits;
